...
struct rxfile {
	...
	struct sockaddr_in6 remote;     /* Peer */
...
static struct rxfile *rxfind(struct sockaddr_in6 *addr) {
	struct rxfile *rx;

	for (rx = rxhead; rx != NULL; rx = rx->next) {
		if (sockaddr6_cmp(&rx->remote, addr) == 0)
			return rx;
	}
...
int eventhandler(rudp_socket_t rsocket, rudp_event_t event, struct sockaddr_in6 *remote) {
	...
	case RUDP_EVENT_CLOSED:
		if (remote && (rx = rxfind(remote))) {
			if (rx->fileopen) {
				fprintf(stderr, "vs_recv: prematurely closed communication with %s:%s\n",
					getnameinfohost(remote), getnameinfoserv(remote));
				close(rx->fd);
			}
			rxdel(rx);
...
int rudp_receiver(rudp_socket_t rsocket, struct sockaddr_in6 *remote, void *buf, int len) {
	...
	case VS_TYPE_BEGIN:
		...
		for (i = 0; i < namelen; i++) {
			unsigned char c = rx->name[i];
			if (!(isalnum(c) || c == '.' || c == '_' || c == '-')) {
				fprintf(stderr, "vs_recv: Illegal file name \"%s\": bad character (hex 0x%x) at position %d\n", 
					rx->name, c, i+1);
				rudp_close(rsocket);
				return 0;
			}
		...
		if (debug) {
			fprintf(stderr, "vs_recv: BEGIN \"%s\" (%d bytes) from %s:%s\n", rx->name, len,
				getnameinfohost(remote), getnameinfoserv(remote));
		}
	...
	case VS_TYPE_DATA:
		if (debug) {
			fprintf(stderr, "vs_recv: DATA (%d bytes) from %s:%s\n", 
				len, 
			getnameinfohost(remote), getnameinfoserv(remote));
		}
	...
	case VS_TYPE_END:
		if (debug) {
			fprintf(stderr, "vs_recv: END (%d bytes) from %s:%s\n",
				len, getnameinfohost(remote), getnameinfoserv(remote));
		}
		...
		break;
	default:
		fprintf(stderr, "vs_recv: bad vsftp type %d from %s:%s\n",
			vs->vs_type, getnameinfohost(remote), getnameinfoserv(remote));
	}
	return 0;
}
