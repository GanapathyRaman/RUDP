...
{
    struct event_data *e, *enew;

    for (e=ee; e; e=e->e_next) {
	if (e->e_type == EVENT_FD) {
	    if (e->e_fd == fd) {
		fprintf(stderr, "event_fd: event handler already registered for file descriptor %d\n", fd);
		return 0;
	    }
	}
    }

    enew = (struct event_data *)malloc(sizeof(struct event_data));
    if (enew==NULL){
	perror("event_fd: malloc");
	return -1;
    }
    memset(enew, 0, sizeof(struct event_data));
    strcpy(enew->e_string, str);
    enew->e_fd = fd;
    enew->e_fn = fn;
    enew->e_arg = arg;
    enew->e_type = EVENT_FD;
    enew->e_next = ee;
    ee = enew;
    return 0;
}
